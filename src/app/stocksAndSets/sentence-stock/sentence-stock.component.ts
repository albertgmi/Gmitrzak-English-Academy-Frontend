...
export class SentenceStockComponent implements OnInit {
    ...
    onRowEditSave(sentence: SentenceStockDto) {

        if (!sentence.polish?.trim() || !sentence.englishTranslation?.trim()) {
            return;
        }

        this.sentenceService.updateStock(
            sentence.id,
            sentence.polish,
        ).subscribe({
            next: () => {
                this.messageService.add({
                    severity: 'success',
                    summary: 'Updated',
                    detail: 'Sentence updated successfully',
                    life: 2000
                });
            },
            error: () => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'Failed to save sentence',
                    life: 3000
                });
    ...
    save() {
        ...
        if (!this.newPolish().trim() || !this.newEnglish().trim()) {
            return;
        }
        ...
        this.sentenceService.uploadStock(file).subscribe({
            next: (r) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Uploaded',
                    detail: `${r.added} sentences added`,
                    life: 3000
                });
    ...
    onFileSelected(event: Event) {
        ...
        this.messageService.add({
            severity: 'info',
            summary: 'Processing',
            detail: 'Translating sentences with AI, this may take a moment...',
            life: 8000
        });
        ...
        this.sentenceService.uploadStock(file).subscribe({
            next: (r) => {
                ...
                this.messageService.add({
                    severity: 'success',
                    summary: 'Uploaded',
                    detail: `${r.added} sentences added and translated`,
                    life: 4000
                });
            ...
            error: (err) => {
                ...
                this.messageService.add({
                    ...
                    detail: err?.error?.message ?? 'Could not process file',
                    life: 4000
                });
    ...
    onDrop(event: DragEvent) {
        ...
        if (file && (file.name.endsWith('.xlsx') || file.name.endsWith('.xls'))) {
            ...
            this.messageService.add({
                severity: 'error',
                summary: 'Invalid file',
                detail: 'Please drop a valid Excel file (.xlsx or .xls)',
                life: 3000
            });
        }
    }
